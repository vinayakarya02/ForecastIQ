...
def validate(df: pd.DataFrame, cfg: Config, logger=None) -> tuple[pd.DataFrame, bool]:
    ...
    def add(name: str, status: str, detail: str = "", rows: int = 0):
        checks.append(
            {"check_name": name, "status": status, "detail": detail, "rows_flagged": rows}
        )
    ...
    if dr and "discount" in df.columns:
        ...
        add(
            ...
            f"{bad} outside [{lo}, {hi}]" if bad else f"within [{lo}, {hi}]",
            bad,
        )
    ...
    offenders = {
        c: round(float(df[c].isna().mean()), 3)
        for c in df.columns
    ...
    add(
        ...
        str(offenders) if offenders else f"all columns <= {max_null}",
        len(offenders),
    )
    ...
    if {"order_date", "ship_date"}.issubset(df.columns):
        ...
        add(
            "ship_after_order",
            "WARN" if bad else "PASS",
            f"{bad} ship_date < order_date" if bad else "ok",
            bad,
        )
    ...
    if {"order_id", "product_id"}.issubset(df.columns):
        ...
        add(
            "unique_order_line",
            "WARN" if bad else "PASS",
            f"{bad} duplicate order_id+product_id" if bad else "unique",
            bad,
        )
