...
c1, c2 = st.columns(2)
with c1:
    charts.show(charts.bar(cat, "category", "revenue", "Revenue by category",
                           labels={"category": "", "revenue": "Revenue ($)"}))
with c2:
    charts.show(charts.bar(cat, "category", "profit_margin_pct", "Profit margin by category (%)",
                           labels={"category": "", "profit_margin_pct": "Margin %"}))
st.dataframe(
    cat.rename(columns={"category": "Category", "revenue": "Revenue", "profit": "Profit",
                        "units": "Units", "orders": "Orders", "profit_margin_pct": "Margin %"}),
...
if not sub.empty:
    ...
    charts.show(charts.bar(view.sort_values("revenue"), "revenue", "sub_category",
                           "Revenue by sub-category", orientation="h", color="category",
...
with tab_top:
    ...
    if not empty_guard(top):
        st.dataframe(
            top.rename(columns={"product_name": "Product", "category": "Category", "revenue": "Revenue",
                                "profit": "Profit", "units": "Units", "profit_margin_pct": "Margin %"}),
            hide_index=True, width="stretch")
...
with tab_low:
    ...
    if low.empty:
        ...
        st.dataframe(
            low.rename(columns={"product_name": "Product", "category": "Category", "revenue": "Revenue",
                                "profit": "Profit", "units": "Units", "profit_margin_pct": "Margin %"}),
            hide_index=True, width="stretch")
