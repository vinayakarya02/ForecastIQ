...
apply_theme()
page_header(
    "Model Performance", "How every forecasting model scored in rolling-origin backtesting", "🏁"
)
...
with c1:
    ...
    charts.show(
        charts.bar(
            wc, "model", "wins", "Series won per model", labels={"model": "", "wins": "Series won"}
        )
    )
with c2:
...
metric = st.radio(
    "Metric", ["mape", "rmse", "mae", "r2"], horizontal=True, key="mp_metric", format_func=str.upper
)
