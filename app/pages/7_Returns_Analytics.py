...
kpi_row([
    ...
    {"label": "Returned Revenue", "value": compact_money(totals["returned_revenue"]),
...
if not empty_guard(trend):
    charts.show(charts.line(trend, "period_start", "return_rate_pct", "Monthly return rate (%)",
...
with c1:
    ...
    if not by_region.empty:
        ...
        charts.show(charts.bar(top.sort_values("return_rate_pct"), "return_rate_pct", "label",
                               "Return rate by region (top 10)", orientation="h",
                               labels={"return_rate_pct": "Return rate %", "label": ""}))
with c2:
    by_cat = da.return_rate_by_category(scope)
    if not by_cat.empty:
        charts.show(charts.bar(by_cat, "category", "return_rate_pct", "Return rate by category",
...
if by_prod.empty:
    ...
    st.dataframe(
        by_prod.rename(columns={"product_name": "Product", "category": "Category", "orders": "Orders",
                                "returned_orders": "Returned", "return_rate_pct": "Return rate %",
