    ...
    with c1:
        charts.show(
            charts.bar(
                ...
                color="rfm_segment",
                labels={"rfm_segment": "", "customers": "Customers"},
            )
        )
    with c2:
...
if not empty_guard(clv):
    ...
    with c1:
        fig = px.histogram(
            clv, x="clv_basic", nbins=40, title="Distribution of customer value (CLV)"
        )
        fig.update_layout(
            xaxis_title="Historical revenue per customer ($)", yaxis_title="Customers"
        )
