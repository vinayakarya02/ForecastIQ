...
charts.show(charts.forecast_chart(result["history"], result["forecast"],
...
with c1:
    ...
    st.dataframe(
        show.rename(columns={"model": "Model", "rmse": "RMSE", "mae": "MAE", "mape": "MAPE %", "r2": "R²"}),
...
with c2:
    ...
    fc = fc.rename(columns={"period_start": "Month", "yhat": "Forecast",
                            "yhat_lower": "Lower", "yhat_upper": "Upper"})
    st.dataframe(fc.style.format({"Forecast": "{:,.0f}", "Lower": "{:,.0f}", "Upper": "{:,.0f}"}),
                 hide_index=True, width="stretch")

download_csv(result["forecast"], f"forecast_{result['series_id'].replace(':', '_')}.csv",
             "Export forecast")
