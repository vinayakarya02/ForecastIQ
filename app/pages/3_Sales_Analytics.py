...
with tab_trends:
    ...
    if grain == "Monthly":
        ...
        rt = da.revenue_trend(scope, window).rename(
            columns={f"revenue_{window}m_avg": "moving_avg"}
        )
        long = rt.melt(
            "period_start", ["revenue", "moving_avg"], var_name="Series", value_name="value"
        )
        long["Series"] = long["Series"].map(
            {"revenue": "Revenue", "moving_avg": f"{window}-mo average"}
        )
        charts.show(
            charts.line(
                long,
                ...
                color="Series",
                labels={"period_start": "", "value": "Revenue ($)"},
            )
        )
    ...
    if frames:
        ...
        charts.show(
            charts.line(
                ...
                "period_start",
                "revenue",
                f"Monthly revenue by {dim.lower()}",
                color=dim,
